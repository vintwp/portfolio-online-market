...
export const Slider: React.FC<Props> = ({
  ...
  const timerAutoplay = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const startIntervalAutoplay = () => {
    timerAutoplay.current = setInterval(() => {
  ...
  const clearIntervalAutoplay = () => {
    if (timerAutoplay.current) {
      clearInterval(timerAutoplay.current);
    }
  };
    ...
    <SliderContext.Provider value={value}>
      <div className={clsx('slider', className && `${className}__slider`)}>
        ...
        {navArrows && (
          <SliderButton
            direction="right"
            disabled={!infinite && activeSlide >= slidesLength - slidesPerSlide}
            className={className}
